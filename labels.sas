  ...
  *      in a scatterplot                                            *
        Doc: http://www.datavis.ca/sasmac/labels.html          
  *------------------------------------------------------------------*
  *  Author:  Michael Friendly         <friendly@yorku.ca>           *
  * Created:  15 May 1991 12:27:15                                   *
  * Revised:  18 Feb 2009 09:28:45                                   *
  * Version:  1.8-1                                                  *
  *  - Added BY= parameter                                           *
  ...
  * 1.8                                                              *
  *  - Renamed to labels to avoid conflict with annomac              *
  *  - Fixed bug with missing values                                 *
  *  - Fixed bug with constant TEXT=                                 *
  *  - Fixed bug with recycling POS=                                 *
...
* CLASS=      The name(s) of any CLASS variable(s) to be used for multiple
              curves within a plot.  The effect of BY= and CLASS= variables
...
* COLOR=      Label color: the name of a dataset character variable, a 
...
%macro labels(
   ...
   class=,         /* name of a curve variable         */
   cvar=,          /* name of a curve variable (syn)   */
...
%if %length(&cvar)=0
    %then %if %length(&class) %then %let cvar=&class;
...
*/
   %else %if %length(&pos) >1 %then    
      %str(position=substr(&pos,1+mod(_n_-1,length(&pos)),1););
...
%if %length(&in) %then %do;
   data &out;
	length function $8;	*-- avoid bug if &in has wrong length;
      set &out &in;
      %if %length(&by) %then %do;
         by &by;
         %end;
