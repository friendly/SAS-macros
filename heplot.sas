  ...
  *   Title: Plot Hypothesis and Error matrices for one MLM effect *
        Doc: http://www.datavis.ca/sasmac/heplot.html       
  *---------------------------------------------------------------*
  *  Author:  Michael Friendly            <friendly@yorku.ca>     *
  * Created:  10 Jan 1997 13:29:31                                *
  * Revised:  24 Aug 2007 11:29:52                                *
  * Version:  1.6-4                                               *
  ...
  *      Pass WIDTH= and HTEXT= to %canvec                        *
  *      Updated %hemeans from %heplots                           *
  *                                                               *
  *---------------------------------------------------------------*/
 /*=
...
%if %length(&data) %then %do;
	...
	*-- Plot the canonical vectors;
	%if %index(%upcase(&add), CANVEC) %then %do;
		%canvec(data=&data, stat=&stat, x=&x, y=&y, effect=&effect, 
			width=&w1, htext=&htext, out=_canvec_);
		%if &syserr > 4 %then %do;
