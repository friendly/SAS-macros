  ...
  *          conditioned on the value of z.                      *
        Doc: http://www.datavis.ca/sasmac/coplot.html      
  *                                                              *
...
data _strip_;
   ...
   x=uppct; y=94;    function='BAR    '; output;
...
%gask;   *-- get hpos, vpos;
	data _boxes_;
		...
		xsys = '2';
		if lowerx=upperx then h=.6;
			else h=.3;
		x=lowerx; y=slice-h;   function='MOVE   '; output;
		x=upperx; y=slice+h;   function='BAR    '; output;
		if last then do;
			xsys = '1'; line=3;
			%do ln = 2 %to &rows;
			y = (&ln - 1)*&cols + .5;
...
%mend;
  ...
  *----------------------------------------------------*/
 /*
%macro gensym(n=1, h=1.5, interp=none,
              symbols=%str(- + : $ = X _ Y),
