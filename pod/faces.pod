...
of variables to facial features using either the C<LEFT=> and C<RIGHT=>
...
The name of the output ANNOTATE data set [Default: C<OUT=ASYM>]

=item ID=

The name of a character ID variable used to
             label the plot cell for a given observation.

=item IDNUM=

The name of a numeric ID variable [Default: C<IDNUM=_N_>]

=item BLKS=

The number of blocks per page [Default: C<BLKS=1>]

=item ROWS=

The number of rows per block [Default: C<ROWS=4>]

=item COLS=

The number of rolumns per block [Default: C<COLS=4>]

=item RES=

Resolution: 1=high/3=low. Higher resolution means that more 
             lines are drawn for each facial feature [Default: C<RES=3>]

=item FRAME=

Draw a frame around each face? [Default: C<FRAME=Y>]

=item COLOR=

Color of each face. Specify a variable name or a string 
             in quotes.  If a variable name is specified, the values 
          are assumed to be SAS color names. [Default: C<COLOR='BLACK'>]

=item HCOLOR=

Hair color.  Specify a variable name, or a string in quotes.
             [Default: C<HCOLOR='BLACK'>]

=item ROW=

The name of an optional variable whose value indicates 
             which row in a block an observation is drawn in.
             The ROW=, COL=, and C<BLK=> parameters may be used to assign 
...
A list of names of (up to) 18 variables to be assigned to 
             features of the left side of the face.  Use a . in C<LEFT=>
          (or C<RIGHT=>) to skip a parameter (leaving it unassigned).
...
Names of individual variables assigned to the left-side features

* item R1= ... R18=

Names of individual variables assigned to the right-side features
...
The name of the graph in the graphic catalog [Default: C<NAME=FACES>]
