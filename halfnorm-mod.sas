...
ods output ObStats=work._obstat_ 
           ParameterEstimates=work.parm(where=(Parameter='Dispersion'));
proc genmod data=&data;
     %if %length(&class)  > 0 %then %do; class &class; %end;
     %if %length(&freq)   > 0 %then %do; freq &freq;   %end;
  	 %if %length(&trials) = 0 %then %do; model &resp         = &model / %end;
	                          %else %do; model &resp/&trials = &model / %end;
...
data _obstat_;
	 ...
	 if _n_  = 1 then set work.parm(keep = estimate);
	 ...
	 do i=1 to dim(_y_);
		...
		%end;
		%else %if %substr(&dist,1,6) = NEGBIN %then %do;
		   teta = 1 / estimate;
...
proc genmod data=_obstat_;
     %if %length(&class)  > 0 %then %do; class &class; %end;
     %if %length(&freq)   > 0 %then %do; freq &freq;   %end;
	 %if %length(&trials) = 0 %then %do; model _y&i         = &model / %end;
                         	  %else %do; model _y&i/&trials = &model / %end;
	 %if %length(&dist)   > 0 %then %do; dist=&dist %end;
	 %if %length(&link)   > 0 %then %do; link=&link %end;
	 %if %length(&offset) > 0 %then %do; offset=&offset  %end;
	 %if %length(&mopt)   > 0 %then %do; %str(&mopt) %end;
