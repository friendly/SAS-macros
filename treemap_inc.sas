...
%macro treemac2(mydata,origdata,id1,id2,origid1,origid2,level,size,treedata,minx,miny,maxx,maxy);

/* Declare all the passed-in variables as 'local' - the only thing
   I'm passing back up to the original program is the 'treedata'
   data set itself. */
%local mydata origdata id1 id2 origid1 origid2 level size treedata minx miny maxx maxy;
...
create table summed&rand as select &id1 length=50, &size from mydata&rand order by &size descending;
quit; run;
...
*/
  ...
  quit; run;
  %treemac2(treefoo&rand,&origdata,id,id,&origid1,&origid2,'inner',&size,&treedata,&minx,&miny,&maxx,&maxy);
 %end; 
 ...
 */
 %treemac2(high&rand,&origdata,&id1,&id2,&origid1,&origid2,&level,&size,&treedata,&minx1,&miny1,&maxx1,&maxy1);
 %treemac2(low&rand, &origdata,&id1,&id2,&origid1,&origid2,&level,&size,&treedata,&minx2,&miny2,&maxx2,&maxy2);
...
%macro treemac(mydata,id1,id2,size,treedata,minx,miny,maxx,maxy);
%local mydata id1 id2 size treedata minx miny maxx maxy;
/*
options mprint;
   proc datasets nolist nowarn; delete &treedata; run;
*/
  %treemac2(&mydata,&mydata,&id1,&id2,&id1,&id2,'outer',&size,&treedata,&minx,&miny,&maxx,&maxy);
%mend treemac;
...
%macro mini_tree(mydata,id,size,treedata,minx,miny,maxx,maxy);
%local mydata id size treedata minx miny maxx maxy;
  data minidata; set &mydata;
     FakeLev1='Lev1';
  run;
  %treemac2(minidata,minidata,FakeLev1,&id,FakeLev1,&id,'outer',&size,&treedata,&minx,&miny,&maxx,&maxy);
%mend mini_tree;
